...
void  newCustomerLogin(roomInformation *rooms){
	...
	rooms[roomCheck-1].customer.customerName=(char*)malloc(sizeof(char)*strlen(customerName)+1);//+1 olmasýnýn sebebei sondaki null karakterinden kaynaklanmaktadir
	...
	ch=getchar();
	if(ch=='N' || ch=='n'){
		cancelReservation(rooms,roomCheck);//eger onaylamazsa kullanici bilgileri odadan silinecek ayrýca bu fonks. musteri odadan ayrilacagi zaman kullanilacak.
		return;
	}
	else if(ch=='Y' || ch=='y'){
	printf("Your new customer entry has been confirmed.");
	rooms[roomCheck-1].roomReservation=1;//musteri onayladigi icin oda durumu artýk dolu.
	}
	else{
